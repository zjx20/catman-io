...
少说了必要的信息会追问：「幫我校個鬧鐘」→「要計幾耐呀？」→ 接着说「三十分鐘」就开始计时——动作用
`ActionResult.ask` 标明在等哪个槽位，下一句先按那个槽位解析，解析不到才当新的一句话。
...
| v3 | -20% ～ +20% | 重采样 0.9～1.1；真人录音 31 条（23 训练 / 8 验证，权重 8） | 100% / 96.1% | 留出 8 条 100%；四折交叉验证 100% | 待测 |
