...
| v2 | -20% ～ +100% | 无；近音短语不再作为负样本 | 100% / 89.1% | 待测 |

各版本都只用合成语音训练，日常句子误接受 ≤ 1%、通用音频每小时误唤醒 ≤ 0.19 次（阈值 0.5）。合成验证集上的数字
并不能代替真机：v1 在合成集上全面领先 v0，真机反而更差。v2 找到了一个具体原因——「小貓銀」「燒貓人」这类只差一个
声调的近音负样本，小分类器分不开，硬压它们会把同一语速档（恰好是正常语速）的正样本一起压掉；v2 不再拿它们训练，
每一档语速的召回都回到 100%，代价是这类近音短语有约 78% 会触发。真人、真麦克风、真房间会打折扣，**最有效的补救**是用目标
