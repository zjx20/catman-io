...
class DataConfig:
    ...
    # 真人录音目录（任意采样率的 wav，自动转 16 kHz 单声道），递归收；子目录名当说话人，
    # evaluate 按它分组报召回。extra_*_dirs 全部训练；想知道真人召回，把每位说话人随机挑几条
    # 放到 extra_*_val_dirs（只验证，evaluate 会用整条录音流式打分、单独报一栏）。
    # 强烈建议录几十条正样本：真人录音是最终的裁判。
